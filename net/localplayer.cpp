...
void CLocalPlayer::RequestClass(int iClass)
{
	...
	pNetGame->GetRakClient()->RPC(&RPC_RequestClass, &bsSpawnRequest, HIGH_PRIORITY, RELIABLE, 0, false, UNASSIGNED_NETWORK_ID, nullptr);
}
...
void CLocalPlayer::SendExitVehicleNotification(VEHICLEID VehicleID)
{
	...
	{ 
		...
		bsSend.Write(VehicleID);
		pNetGame->GetRakClient()->RPC(&RPC_ExitVehicle, &bsSend, HIGH_PRIORITY, RELIABLE_SEQUENCED, 0, false, UNASSIGNED_NETWORK_ID, nullptr); // NULL
	}
}
...
void CLocalPlayer::SendOnFootFullSyncData()
{
	...
	ofSync.byteCurrentWeapon = 0; // fix this later m_pPlayerPed->GetCurrentWeapon();
	ofSync.byteSpecialAction = 0; // fix this later
...
void CLocalPlayer::ProcessSpectating()
{
	...
	if(m_byteSpectateType == SPECTATE_TYPE_NONE)
	{
		...
		pGame->GetCamera()->SetPosition(50.0f, 50.0f, 50.0f, 0.0f, 0.0f, 0.0f); // Default OnServerJoin coords??
