...
void CRemotePlayer::Process()
{
	...
	if(IsActive()) // It seems to be like entity is out of streaming.
	{
			...
			m_byteUpdateFromNetwork == UPDATE_TYPE_INCAR && m_pPlayerPed->IsInVehicle())
		{
			...
			{
				// maybe need to do any other code
			...
			m_byteUpdateFromNetwork == UPDATE_TYPE_PASSENGER /* && m_pPlayerPed->IsInVehicle() */) // I don't know
		{
		...
		if(GetState() == PLAYER_STATE_ONFOOT && !m_pPlayerPed->IsInVehicle())
		{
			...
			if((GetTickCount() - m_dwLastRecvTick) > 1500) // what is that magic number?
				m_bIsAFK = true;
		...
		//if(m_byteState != PLAYER_STATE_WASTED) // wtf? Is it going to be like m_byteState == PLAYER_STATE_SPAWNED?
			//m_pPlayerPed->SetHealth(1000.0f);
			
		if (m_byteState == PLAYER_STATE_WASTED) // remake by Solpadoin. Should sync health now.
			m_pPlayerPed->SetHealth(0.0f);
		else
		{
			float reportedHealth = GetReportedHealth();
			
			if (reportedHealth > 0 && reportedHealth <= 1000)
				m_pPlayerPed->SetHealth(reportedHealth);
			else
				m_pPlayerPed->SetHealth(1000.0f);
		}
		
		m_pPlayerPed->SetArmour(GetReportedArmour());
...
bool CRemotePlayer::Spawn(uint8_t byteTeam, unsigned int iSkin, VECTOR *vecPos, float fRotation, 
	uint32_t dwColor, uint8_t byteFightingStyle, bool bVisible) // trying to optimize it for mobile devices
{
	if(m_pPlayerPed)
	{
		pGame->RemovePlayer(m_pPlayerPed);
	...
	// optimization
	/*
	if(pPlayer && pPlayer->GetDistanceFromCamera() >= (pNetGame->m_fNameTagDrawDistance * 4) )
	{
		if(m_pPlayerPed)
		{
			pGame->RemovePlayer(m_pPlayerPed);
	...
	*/
	...
	if(pPlayer)
	{
		bool isPedStreamed = pPlayer->GetDistanceFromCamera() <= 200.f; //(pNetGame->m_fNameTagDrawDistance * 4);
...
void CRemotePlayer::StoreOnFootFullSyncData(ONFOOT_SYNC_DATA *pofSync, uint32_t dwTime)
{
	if( !dwTime || (dwTime - m_dwUnkTime) >= 0 )
	{
		...
		m_fReportedHealth = (float)pofSync->byteHealth; // get reported Health
		m_fReportedArmour = (float)pofSync->byteArmour; // get reported Armour
		...
		if(m_pPlayerPed)
		{
			if(m_pPlayerPed->IsInVehicle())
			{
				...
				m_byteSpecialAction != SPECIAL_ACTION_EXIT_VEHICLE /*&& !sub_100A6F00()*/) // reverse this later
					RemoveFromVehicle();
			}
		}
...
void CRemotePlayer::StoreInCarFullSyncData(INCAR_SYNC_DATA *picSync, uint32_t dwTime)
{
	if(!dwTime || (dwTime - m_dwUnkTime >= 0))
	{
		...
		m_byteSeatID = 0; // seat 0 is a driver seat. This method is used only for driver's so it always should be zero.
		
		m_fReportedHealth = (float)picSync->bytePlayerHealth; // get reported player health
		m_fReportedArmour = (float)picSync->bytePlayerArmour; // get reported player armour
