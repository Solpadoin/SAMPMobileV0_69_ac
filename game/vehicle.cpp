...
CVehicle::CVehicle(int iType, float fPosX, float fPosY, float fPosZ, float fRotation, bool bSiren)
{
		...
		(iType != TRAIN_TRAM)) 
	{
		...
		if(m_pVehicle)
		{
			m_pVehicle->dwDoorsLocked = 0; // sync it later
			m_bIsLocked = false;
...
bool CVehicle::IsDriverLocalPlayer()
{
	if(m_pVehicle)
	{
		if((PED_TYPE*)m_pVehicle->pDriver == GamePool_FindPlayerPed()) // Maybe it causes bugs
			return true;
	}
...
void CVehicle::ProcessMarkers()
{
	...
	if(GetDistanceFromLocalPlayerPed() < 200.0f && !IsOccupied()) // default distance is 200.0f
	{
		if(!m_dwMarkerID)
		{
			// show
	...
	else if(IsOccupied() || GetDistanceFromLocalPlayerPed() >= 200.0f) // 200.f
	{
		// remove
		if(m_dwMarkerID)
		{
