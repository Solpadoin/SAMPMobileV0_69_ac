...
void CPlayerTags::Render()
{
	...
	if(pNetGame && pNetGame->m_bShowPlayerTags)
	{
		...
		{
			if(pPlayerPool && pPlayerPool->GetSlotState(playerId) == true)
			{
				...
				if(pPlayer && pPlayer->IsActive() && pPlayer->m_bShowNameTag)
				{
					...
					auto distToPed = pPlayerPed->GetDistanceFromCamera();
					
					if(pPlayerPed && distToPed <= pNetGame->m_fNameTagDrawDistance)
					{
