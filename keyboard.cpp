	...
	for(int i = 0; i < 4; i++)
	{
		for( auto key : m_Rows[m_iLayout][i])
		{
			...
			switch(key.type)
			{
				...
				case KEY_SWITCH: // small addon by Solpadoin
					if (m_iLayout == LAYOUT_ENG)
						ImGui::GetOverlayDrawList()->AddText(pGUI->GetFont(), m_fFontSize, key.symPos, 0xFFFFFFFF, "en");
					else if (m_iLayout == LAYOUT_RUS)
						ImGui::GetOverlayDrawList()->AddText(pGUI->GetFont(), m_fFontSize, key.symPos, 0xFFFFFFFF, "ru");
					else if (m_iLayout == LAYOUT_NUM)
...
void CKeyBoard::InitENG()
{
	...
	key.id++;
	if (row != nullptr) row->push_back(key);
	curPos.x += key.width;
